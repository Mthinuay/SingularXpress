...
export const editEmployee = async (employeeNumber, employee) => {
  try {
    const response = await axios.put(`${API_BASE}/edit/${employeeNumber}`, employee, {
      headers: { "Content-Type": "application/json" },
    });
    return response.data || {};
  } catch (error) {
...
export const fetchEmployeeByIdNumber = async (idNumber) => {
  try {
    const response = await axios.get(`${API_BASE}/by-idnumber/${encodeURIComponent(idNumber)}`);
...
export const populateFromIdNumber = (idNumber) => { 
  ...
  try {
    ...
    console.log({ dateOfBirth: dob.toLocaleDateString("en-CA"), gender, nationality });
...
export const handleInputChange = (e, employee, setEmployee, setIdNumberError, employeesList = []) => {
